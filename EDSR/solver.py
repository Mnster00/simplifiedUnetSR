...
class EDSRTrainer(object):
    ...
    def build_model(self):
        self.model = Net(num_channels=3, upscale_factor=self.upscale_factor, base_channel=64, num_residuals=4).to(self.device)
        ...
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[ 50,75,100,150,200,300,400], gamma=0.5)  # lr decay
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_num, (data, target) in enumerate(self.testing_loader):
                ...
                mse = self.criterion_mse(prediction, target)
